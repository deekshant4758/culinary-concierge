...
function canViewOrder(user, order) {
  ...
  if (order.collaborators?.some((c) => c.userId === user.id)) return true;
  if (user.role === 'manager' && order.region === user.region) return true;
  return false;
}

function canEditDraft(user, order, collaborators = []) {
  if (!user || !order) return false;
  if (user.role === 'admin') return true;
  if (order.userId === user.id) return true;
  // Check if user is in collaborators list
  return collaborators.some(c => c.userId === user.id);
}
    ...
    const existingDraft = await tx.order.findFirst({
      where: draftOrderId
        ...
        : {
            ...
            status: 'draft',
            OR: [{ userId: actor.id }, { collaborators: { some: { userId: actor.id } } }],
          },
...
export const resolvers = {
  ...
  Order: {
    ...
    paymentMethod: (o) => o.paymentMethodId
      ...
      : null,
    collaborators: async (o) => {
      if (o.collaborators) {
        return o.collaborators.map(c => c.user || prisma.user.findUnique({ where: { id: c.userId } }));
      }
      const collabs = await prisma.orderCollaborator.findMany({
        where: { orderId: o.id },
        include: { user: true },
      });
      return collabs.map(c => c.user);
    },
  ...
  Query: {
    ...
    order: async (_, { id }, { user }) => {
      ...
      const o = await prisma.order.findUnique({
        where: { id },
        include: { restaurant: true, user: true, collaborators: { include: { user: true } }, paymentMethod: true, items: { include: { menuItem: true } } },
      });
  ...
  Mutation: {
    ...
    shareDraftOrder: async (_, { id, userId }, { user }) => {
      ...
      const order = await prisma.order.findUnique({
        where: { id },
        include: { user: true, collaborators: true },
      });
      if (!order) throw new GraphQLError('Order not found.', { extensions: { code: 'NOT_FOUND' } });
      if (order.status !== 'draft') {
      ...
      const alreadyCollaborator = order.collaborators.some(c => c.userId === userId);
      if (alreadyCollaborator) {
        throw new GraphQLError('This user is already a collaborator on this draft.', { extensions: { code: 'BAD_REQUEST' } });
      }

      // Add collaborator
      await prisma.orderCollaborator.create({
        data: { orderId: id, userId },
      });
    ...
    removeCollaborator: async (_, { id, userId }, { user }) => {
      requireAuth(user);
      ...
      // Delete the collaborator relationship
